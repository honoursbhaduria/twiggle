        ...
        with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for el in elements:
                ...
                writer.writerow([
                    ...
                    tags.get("addr:state", "") or tags.get("is_in:state", ""),
                    tags.get("addr:country", "") or tags.get("is_in:country", ""),
                    tags.get("addr:full", "") or tags.get("addr:street", ""),
                    tags.get("description", "") or tags.get("note", ""),
                    tags.get("wikidata", ""),
                    str(tags)
                ])

        self.stdout.write(
            self.style.SUCCESS(f"✅ Saved {len(elements)} attractions to {output_file}")
        )
